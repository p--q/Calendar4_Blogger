// Calendar4_Bloggerモジュール
var Calendar4_Blogger = Calendar4_Blogger || function() {
    var cl = {
        callback: {  // コールバック関数。
    ...
    var cal = {  // カレンダーを作成するオブジェクト。
        ...
        createURL: function(max) {  // フィードを取得するためのURLを作成。
            ...
            url += "&callback=Calendar4_Blogger.callback.getArticles&max-results=" + vars.max;  // コールバック関数と最大取得投稿数を設定。
...
Calendar4_Blogger.all("calendar4_blogger");  // idがcalendar_bloggerの要素にカレンダーを表示させる。
